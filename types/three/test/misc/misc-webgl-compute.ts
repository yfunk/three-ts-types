...
import { Vector2Uniform } from 'three/examples/jsm/renderers/webgpu/WebGPUUniform';
...
type ComputeParamsImpl = {
    num: number;
    shader: string;
    bindings: Array<WebGPUUniformsGroup | WebGPUStorageBuffer>;
};

const computeParams: ComputeParamsImpl[] = [];
...
async function init() {
    ...
    const pointerGroup = new WebGPUUniformsGroup('mouseUniforms').addUniform(new Vector2Uniform('pointer', pointer));
    ...
    const pointsGeometry = new THREE.BufferGeometry().setAttribute('position', particleBuffer.attribute);
