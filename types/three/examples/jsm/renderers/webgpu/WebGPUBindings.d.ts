...
export default class WebGPUBindings {
    ...
    constructor(device, info: WebGPUInfo, properties, textures, pipelines, computePipelines, attributes, nodes);
        ...
        for (const binding of bindings) {
            ...
            if (binding.isUniformsGroup) {
                ...
                if (needsBufferWrite === true) {
                    this.device.queue.writeBuffer(bufferGPU, 0, array, 0);
                }
        ...
        for (const binding of bindings) {
            if (binding.isUniformsGroup) {
                ...
                entries.push({
                    ...
                    resource: binding.textureGPU.createView({ dimension: binding.dimension }),
                });
            }
